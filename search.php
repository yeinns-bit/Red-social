	...
	?>
		...
		<?php
		    if ($friend_id == $session_id) {
		        ...
		        $stmt1 = $conn->prepare("SELECT * FROM friends WHERE (my_id = :session_id AND my_friend_id = :friend_id) OR (my_id = :friend_id AND my_friend_id = :session_id)");
		        $stmt1->execute(['session_id' => $session_id, 'friend_id' => $friend_id]);
		        $count1 = $stmt1->rowCount();
		        if ($count1 > 0) {
		            echo '<a class="btn btn-success" href="friend_photos.php?friend_id=' . $friend_id . '"><i class="icon-picture"></i> Ver Fotos</a>';
		        } else {
		?>
